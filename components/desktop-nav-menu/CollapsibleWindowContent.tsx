import { inriaSerif, inriaSans } from '@/app/fonts';
...
const DescriptionSection = ({
  ...
  return (
    <div className="w-[35%] bg-neutral-100 py-17 pl-8 text-white flex items-center justify-center">
      <div className='w-[80%]'>
        <h1 className={cn('text-5xl mb-10 font-light tracking-tight', inriaSerif.className)}>{header}</h1>
...
const LinksSection = ({ links }: { links: Link[] }) => {
  return (
    <div className="flex-1 grid grid-cols-[repeat(3,220px)] gap-x-7 gap-y-8 place-content-center place-items-center text-sm font-semibold">
      {links.map((link) => (
...
export default function CollapsibleWindowContent({
  description,
  header,
  links,
  genericLink,
